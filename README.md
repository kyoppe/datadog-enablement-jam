...
- 対象モジュール: **APM Troubleshoot Workflow**。
- APM 調査は **Trace Search ではなく APM Service Page** を起点にする。
  各クエストには「Datadog で開く」リンクがあり、`env:dej` で絞り込んだ Service Page /
  Error Tracking がワンクリックで開きます。
- 教える調査導線:
  `APM Service Page -> レイテンシ / エラー率 / リクエスト数 -> Resource ごとのレイテンシ -> 下流依存 / Trace Sample（フレームグラフ）-> Error Tracking -> 原因サービス / Resource`
- 現役クエスト（3 本、すべて `env:dej` の実テレメトリ。p95 が最大の商品ページを先頭に出題）:

| # | クエスト | 症状 | 原因サービス | Resource |
|---|---|---|---|---|
| 1 | 商品ページ遅延 (`apm-product-ads`) | 商品詳細が最遅 (p95 最大) | `ads-service`（GET /ads が支配的, ~78%） | `Spree::ProductsController#show` |
| 2 | ホームページ遅延 (`apm-home-discounts`) | トップページが遅い | `discounts-service`（GET /discount が支配的, ~88%） | `Spree::HomeController#index` |
| 3 | テンプレートエラー (`apm-store-errors`) | エラー多発 | `store-frontend` 自身 | `spree/shared/_head.html.erb` |

> クエスト 3 は `ActionView::Template::Error` が最多。下流依存ではなく store-frontend
> 自身のテンプレート描画エラーである点が切り分けのポイント。
...
- `DEJ_DATADOG_LOGIN_*` … 参加者がロビーで使う **共有 Datadog ログイン**（URL / メール /
  パスワード）。Datadog アカウントは MVP では **1 つの共有アカウントを貸し出す** 方式のため、
  参加者ごとの org ユーザ作成（プロビジョニング）は行いません。サーバ専用
  `/api/datadog-login` 経由で配信し、ブラウザバンドルには焼き込みません。
...
2. **参加者用 URL** を共有。参加者は **表示名のみ**（メール不要）を登録します。
   入力した名前はリーダーボードにそのまま表示されます（匿名表示はしません）。
...
6. 参加者が APM で調査 → 回答送信 → スコア更新。各参加者は「回答を終了する」を押すと
   その時点でスコアを確定でき、以降は回答・ヒント取得ができなくなります（リーダーボードに「回答終了」表示）。
7. **リーダーボード**（Web ライブ / Datadog ダッシュボード）で進捗を共有。
8. **セッション終了**（Admin、running→ended）。回答は締め切られ、リーダーボードは残ります。

---

## Datadog での調査方法（クエスト 1：商品ページ遅延の例）

1. クエストの **「Datadog で開く」** リンク（`store-frontend` の Service Page、`env:dej` 絞り込み済み）を開く。
2. **レイテンシ / エラー率 / リクエスト数** でレイテンシ悪化を確認する。
3. **Resource ごとのレイテンシ** で最も遅い Resource を探す。
   - 該当: `Spree::ProductsController#show`（商品詳細ページ、p95 が最大）
4. その Resource の **Trace Sample**（フレームグラフ）を開き、下流サービスの **Span Duration** を比較する。
5. 支配的な下流サービス = **`ads-service`**（`GET /ads` がリクエスト時間の ~78%）を特定する。

> クエスト 2（ホーム遅延）も同じ導線で、Trace Sample から `discounts-service`
> （`GET /discount` が ~88%）を特定します。
> クエスト 3（エラー）は **Error Tracking**（`service:store-frontend env:dej` 絞り込み済みリンク）
> を起点に、最多エラー `ActionView::Template::Error` の発生 Resource
> `spree/shared/_head.html.erb` を特定します（下流ではなく store-frontend 自身のエラー）。

---

## 期待する回答（3 クエスト）

| クエスト | 原因サービス | Resource / Endpoint |
|---|---|---|
| 商品ページ遅延 | `ads-service` | `Spree::ProductsController#show` |
| ホームページ遅延 | `discounts-service` | `Spree::HomeController#index` |
| テンプレートエラー | `store-frontend` | `spree/shared/_head.html.erb` |

根拠となる Datadog URL は任意。回答の一致判定は **大文字小文字を無視 / 前後空白をトリム**
した完全一致です（`scoring.ts`）。
...
- 各正解要素は **1 回だけ** 加点されます。
- スコアの定義は各クエストの YAML（[`config/quests/`](config/quests/) 配下、例:
  `apm-home-discounts.yaml`）に集約されており、UI にハードコードしていません。
