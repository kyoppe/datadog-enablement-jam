...
- [x] **モジュール/クエストのリネーム・拡張**: モジュールを `apm-troubleshoot-workflow`（**APM Troubleshoot Workflow**）にリネームし、クエストを 3 本に拡張（`apm-home-discounts` / `apm-product-ads` / `apm-store-errors`）。旧 `apm-service-page-basics` / `apm-slow-checkout` は削除
- [x] **参加者登録からメール廃止**: 共有 Datadog アカウント貸し出し前提のため、登録は **表示名のみ**。per-user プロビジョニングは行わない
- [x] **リーダーボードの匿名化を廃止**: 入力した表示名をそのまま表示。メトリクス / RUM タグも `dej_player=<表示名>`（`dej_handle` / `dej_email` 廃止、多バイト文字可）
...
### 参加者識別（MVP は表示名のみ → 将来は本格的な仕組み）

- MVP の現状: 参加者の識別は **入力した表示名のみ**。IP / cookie / localStorage は未使用。
  サーバは `${sessionId}:${表示名}` でプレイヤーを管理し、同名を再入力すると同一プレイヤーに合流する
  （別人が同名を入れると衝突し得る／リロードで名前が消えて再入力が必要）。
- [ ] 将来: 何らかの **ユーザ識別の仕組み** を導入する。候補:
  - ブラウザ側に匿名トークンを発行して cookie / localStorage に保持し、リロード後も同一プレイヤーを維持
  - 参加用 URL に per-player トークンを埋め込む（TEM が一意な URL を配布）
  - Datadog org ユーザ（共有→個別）や OAuth/SSO 連携と紐付け（Org 移行とあわせて検討）
- [ ] 同名衝突の防止（セッション内で表示名のユニーク制約、または衝突時の枝番付与）
