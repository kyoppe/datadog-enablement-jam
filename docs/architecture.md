...
```
        ...
        |  - dej.score メトリクス      | ─► |     -> ads-service (sleep)          |
        |  - dej.player.logged_in      |    |  postgres                           |
...
- Datadog 連携（サーバ側）: `apps/web/src/lib/datadog-server.ts`
  - `dej.score` … プレイヤー別・クエスト別スコアの time-series。
  - `dej.player.logged_in` … ロビーでのログイン確認。司会用の QueryValue 可視化に使う。
...
| `running` | 司会が「問題スタート」を押した | クエスト（調査・回答） |
...
- `POST /api/login` - ロビーでのログイン確認（`dej.player.logged_in` 送信）
...
- `config/quests/apm-slow-checkout.yaml`（store-frontend → discounts-service の遅延）
